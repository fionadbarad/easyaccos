...
export default function YearTracker() {
  ...
  return (
    <div className="bg-[var(--sa-surface)] border border-[var(--sa-border)] rounded-md overflow-hidden">
      <div className="px-6 py-5 border-b border-[var(--sa-border)] flex items-start justify-between gap-4 flex-wrap">
        <div>
        ...
        <button
          ...
          className="bg-transparent border border-[var(--sa-border)] rounded px-[10px] py-[5px] cursor-pointer text-[rgba(244,245,248,0.42)] text-[0.72rem] font-mono uppercase tracking-[0.08em] whitespace-nowrap transition-all duration-150 ease-in-out"
      ...
      <AnimatePresence>
        {showSetup && (
          ...
          >
            <div className="px-6 py-5 grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4">
              <div>
      ...
      <div className="p-6 flex flex-col gap-6">
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-[repeat(auto-fit,minmax(130px,1fr))] gap-[1px] border border-[var(--sa-border)] rounded-md overflow-hidden bg-[var(--sa-border)]">
              {[
                ...
                <div key={label} className="bg-[var(--sa-surface)] px-[1.1rem] py-[1rem]">
                  <div className="text-[rgba(244,245,248,0.18)] text-[0.58rem] uppercase tracking-[0.08em] font-semibold mb-1.5 font-mono">{label}</div>
                  <div className="text-[var(--sa-white)] text-[1.15rem] font-semibold tracking-[-0.03em] tabular-nums leading-none">{value}</div>
                  <div className="text-[rgba(244,245,248,0.18)] text-[0.6rem] mt-1">{sub}</div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-4">
              ...
              <div className="bg-[var(--sa-black)] border border-[var(--sa-border)] rounded-md px-[1.25rem] py-[1.1rem]">
                <div className="text-[rgba(244,245,248,0.18)] text-[0.6rem] uppercase tracking-[0.1em] font-semibold mb-4 font-mono">Projected tax bill (Jan 2028)</div>
                {projectedTax ? (
                  <>
                    <div className="text-[var(--sa-white)] text-[1.8rem] font-semibold tracking-[-0.04em] leading-none tabular-nums mb-4">
                      {fmt(projectedBill)}
                    </div>
                    ...
                    {projectedTax.sixtyPercentTrap && (
                      <div className="mt-3 px-[10px] py-[8px] bg-[rgba(251,191,36,0.06)] border border-[rgba(251,191,36,0.2)] rounded flex gap-[7px]">
              ...
              <div className="bg-[var(--sa-black)] border border-[var(--sa-border)] rounded-md px-[1.25rem] py-[1.1rem] flex flex-col gap-4">
                ...
                {projectedTax && (
                  <div>
                    ...
                    <motion.div
                      ...
                      className={`px-[14px] py-[12px] rounded-md flex items-start gap-2.5 ${isOnTrack ? 'bg-[rgba(74,222,128,0.06)] border border-[rgba(74,222,128,0.2)]' : 'bg-[rgba(248,113,113,0.06)] border border-[rgba(248,113,113,0.2)]'}`}
                    >
                      {isOnTrack
                      ...
                      <div>
                        {isOnTrack ? (
                          ...
                          <>
                            <div className="text-[#F87171] text-[0.82rem] font-semibold mb-0.5">
                              Shortfall: {fmt(gap)}
                            </div>
                            <div className="text-[rgba(244,245,248,0.42)] text-[0.72rem] leading-[1.5]">
                              {monthsRemaining > 0
            ...
            <div className="bg-[var(--sa-gray)] rounded-md px-[1.25rem] py-[1rem] flex items-center justify-between flex-wrap gap-3 border border-[var(--sa-border)]">
              <div>
