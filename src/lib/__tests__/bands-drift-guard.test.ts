...
const FORBIDDEN_LITERALS: Array<{ pattern: RegExp; reason: string; useInstead: string }> = [
  { pattern: /\b8\.75\s*%/,                 reason: 'pre-2026 dividend basic rate',          useInstead: 'pct(DIV_BASIC) from bands-2026 (now 10.75%)' },
  { pattern: /\b33\.75\s*%/,                reason: 'pre-2026 dividend higher rate',         useInstead: 'pct(DIV_HIGHER) from bands-2026 (now 35.75%)' },
  { pattern: /\b13\.8\s*%/,                 reason: 'pre-2026 employer NI rate',             useInstead: 'pct(EMPLOYER_NI_RATE) from bands-2026 (now 15%)' },
  { pattern: /£\s*12[,_]?570\b/,            reason: 'PA / NI Primary Threshold literal',     useInstead: 'fmtGBP(PA_BASE) / fmtGBP(NI_PT)' },
  { pattern: /£\s*50[,_]?270\b/,            reason: 'NI UEL / basic rate ceiling literal',   useInstead: 'fmtGBP(RUK_BASIC_LIMIT) / fmtGBP(NI_UEL)' },
  { pattern: /£\s*100[,_]?000\b/,           reason: 'PA taper start literal',                useInstead: 'fmtGBP(PA_TAPER_START)' },
  { pattern: /£\s*125[,_]?140\b/,           reason: 'PA taper end / additional rate threshold', useInstead: 'fmtGBP(PA_TAPER_END) / fmtGBP(RUK_HIGHER_LIMIT)' },
  { pattern: /£\s*37[,_]?700\b/,            reason: 'rUK basic rate band width literal',     useInstead: 'fmtGBP(RUK_BASIC_RATE_WIDTH)' },
  { pattern: /£\s*30[,_]?000\s*(rule|exemption|redundancy)/i, reason: 'redundancy exemption literal', useInstead: 'fmtGBP(REDUNDANCY_EXEMPTION)' },
  { pattern: /£\s*7[,_]?105\b/,             reason: 'NI Class 2 SPT literal',                useInstead: 'fmtGBP(NI_CLASS2_SPT)' },
]
