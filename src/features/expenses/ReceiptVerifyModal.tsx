      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.25rem', borderBottom: `1px solid ${C.border}` }}>
          <div>
            ...
            <h3 style={{ color: C.white, fontSize: '0.95rem', fontWeight: 600, margin: 0 }}>Verify Receipt Details</h3>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, overflow: 'hidden', minHeight: 0 }}>
          <div style={{ borderRight: `1px solid ${C.border}`, overflowY: 'auto', background: C.bg, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '1rem' }}>
            {isImage ? (
              ...
              <img src={scan.extract.imageUrl} alt="Receipt" style={{ maxWidth: '100%', borderRadius: '4px', border: `1px solid ${C.border}` }} />
            ) : (
          ...
          <div style={{ overflowY: 'auto', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div>
              ...
              <input type="text" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} style={{ ...inputS, fontFamily: 'inherit' }} />
            </div>
            <div>
              <label style={labelS}>
                Category
            ...
            <div>
              ...
              <input type="number" min={0} step={0.01} value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} placeholder="0.00" style={inputS} />
            </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', padding: '0.875rem 1.25rem', borderTop: `1px solid ${C.border}` }}>
          <button onClick={onCancel} style={{ background: 'transparent', color: C.muted, border: `1px solid ${C.border}`, borderRadius: '4px', padding: '8px 16px', cursor: 'pointer', fontSize: '0.8rem' }}>
            Cancel
          </button>
          <button onClick={() => onConfirm(form)} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: C.white, color: C.bg, border: 'none', borderRadius: '4px', padding: '8px 18px', fontWeight: 600, cursor: 'pointer', fontSize: '0.8rem' }}>
