...
export default function YearTracker() {
  ...
  return (
    <div className="bg-[#1C1D20] border border-[rgba(244,245,248,0.07)] rounded-[6px] overflow-hidden">
      <div className="px-6 py-5 border-b border-[rgba(244,245,248,0.07)] flex items-start justify-between gap-[1rem] flex-wrap">
        <div>
          <div className="flex items-center gap-[8px] mb-[3px]">
            <Calendar size={14} className="text-[rgba(244,245,248,0.42)]" />
            <span className="text-[#F4F5F8] text-[0.9rem] font-semibold tracking-[-0.02em]">2026/27 Tax Year Tracker</span>
          </div>
        ...
        <button
          ...
          className="bg-transparent border border-[rgba(244,245,248,0.07)] rounded-[4px] px-[10px] py-[5px] cursor-pointer text-[rgba(244,245,248,0.42)] text-[0.72rem] font-mono uppercase tracking-[0.08em] whitespace-nowrap transition-all duration-150 ease-in-out"
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(244,245,248,0.3)'; (e.currentTarget as HTMLElement).style.color = '#F4F5F8' }}
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(244,245,248,0.07)'; (e.currentTarget as HTMLElement).style.color = 'rgba(244,245,248,0.42)' }}
        >
          Setup
        </button>
      </div>
      ...
      <AnimatePresence>
        {showSetup && (
          <motion.div
            ...
            className="overflow-hidden border-b border-[rgba(244,245,248,0.07)]"
          >
      ...
      <div className="p-6 flex flex-col gap-[1.5rem]">
        ...
        <div>
          <div className="flex justify-between mb-[8px]">
            ...
            <span className="text-[#F4F5F8] text-[0.65rem] font-mono font-semibold">
              {Math.round(yearProgress * 100)}% · {daysElapsed}d elapsed
            </span>
          </div>
          <div className="h-[4px] bg-[#222326] rounded-[2px] overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${yearProgress * 100}%` }}
              transition={{ duration: 1, ease: 'easeOut' }}
              style={{ height: '100%', background: yearProgress > 0.75 ? '#FBBF24' : '#F4F5F8', borderRadius: '2px' }}
            />
          </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-[repeat(auto-fit,minmax(130px,1fr))] gap-[1px] border border-[rgba(244,245,248,0.07)] rounded-[6px] overflow-hidden bg-[rgba(244,245,248,0.07)]">
              {[
                ...
                <div key={label} className="bg-[#1C1D20] px-[1.1rem] py-[1rem]">
                  <div className="text-[rgba(244,245,248,0.18)] text-[0.58rem] uppercase tracking-[0.08em] font-semibold mb-[6px] font-mono">{label}</div>
                  <div className="text-[#F4F5F8] text-[1.15rem] font-semibold tracking-[-0.03em] tabular-nums leading-none">{value}</div>
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-[1rem]">
              ...
              <div className="bg-[#181818] border border-[rgba(244,245,248,0.07)] rounded-[6px] px-[1.25rem] py-[1.1rem]">
                ...
                {projectedTax ? (
                  <>
                    <div className="text-[#F4F5F8] text-[1.8rem] font-semibold tracking-[-0.04em] leading-none tabular-nums mb-[1rem]">
                      {fmt(projectedBill)}
                    </div>
                    <div className="flex flex-col gap-0">
                      ...
                      <div className="flex justify-between pt-[8px]">
                        ...
                        <span className="text-[#F4F5F8] text-[0.72rem] font-mono">{projectedTax.effectiveTaxRate.toFixed(1)}%</span>
                      </div>
                    </div>
              ...
              <div className="bg-[#181818] border border-[rgba(244,245,248,0.07)] rounded-[6px] px-[1.25rem] py-[1.1rem] flex flex-col gap-[1rem]">
                ...
                {projectedTax && (
                  <div>
                    <div className="mb-[10px]">
                      ...
                      <div className="h-[6px] bg-[#222326] rounded-[3px] overflow-hidden">
                        <motion.div
                      ...
                      <div>
                        {isOnTrack ? (
                          ...
                          <>
                            ...
                            <div className="text-[rgba(244,245,248,0.42)] text-[0.72rem] leading-[1.5]">
                              {monthsRemaining > 0
                                ? <>Save <strong className="text-[#F4F5F8]">{fmt(monthlyNeeded)}/month</strong> for the next {Math.round(monthsRemaining)} months to cover your bill.</>
            ...
            <div className="bg-[#222326] rounded-[6px] px-[1.25rem] py-[1rem] flex items-center justify-between flex-wrap gap-[0.75rem] border border-[rgba(244,245,248,0.07)]">
              <div>
                ...
                <div className="text-[#F4F5F8] text-[0.9rem] font-semibold tracking-[-0.02em]">31 January 2028</div>
              </div>
