      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '2rem' }}>
        {CATEGORIES.map((cat) => (
          <button key={cat} onClick={() => changeCategory(cat)}
            style={{ padding: '6px 16px', borderRadius: '999px', border: `1px solid ${category === cat ? C.gold : C.border}`, background: category === cat ? 'rgba(194,163,104,0.15)' : 'transparent', color: category === cat ? C.gold : C.muted, cursor: 'pointer', fontSize: '0.8rem', fontWeight: 600, transition: 'all 0.15s' }}>
            {cat}
          </button>
        ))}
      </div>
        ...
        <>
          ...
          <div style={{ marginBottom: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
              ...
              <span style={{ color: C.gold, fontSize: '0.78rem' }}>{Math.round(progress)}%</span>
            </div>
            <div style={{ height: '4px', background: 'rgba(255,255,255,0.07)', borderRadius: '2px', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${progress}%`, background: C.gold, borderRadius: '2px', transition: 'width 0.3s ease' }} />
            </div>
          </div>
          ...
          <div onClick={() => setFlipped((f) => !f)}
            style={{
              background: C.card, border: `1px solid ${flipped ? C.gold : C.border}`,
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '1rem' }}>
                <span style={{ padding: '3px 10px', background: 'rgba(194,163,104,0.12)', border: `1px solid ${C.border}`, borderRadius: '999px', color: C.gold, fontSize: '0.7rem', letterSpacing: '0.08em' }}>
                  {card.category}
                </span>
              </div>
              <h2 style={{ fontFamily: 'var(--font-playfair)', color: C.text, fontSize: '1.35rem', fontWeight: 700, marginBottom: '1rem' }}>
                {card.title}
              </h2>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1.5rem' }}>
              ...
              <button onClick={(e) => { e.stopPropagation(); setExpanded(card) }}
                style={{ display: 'flex', alignItems: 'center', gap: '5px', color: C.gold, background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem' }}>
            <button onClick={prev} disabled={idx === 0}
              style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 20px', background: idx === 0 ? 'transparent' : 'rgba(194,163,104,0.1)', border: `1px solid ${idx === 0 ? C.border : C.gold}`, borderRadius: '5px', color: idx === 0 ? C.muted : C.gold, cursor: idx === 0 ? 'default' : 'pointer', fontSize: '0.875rem', fontWeight: 600 }}>
              <ChevronLeft size={16} /> Previous
            </button>
            <button onClick={next} disabled={idx === filtered.length - 1}
              style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 20px', background: idx === filtered.length - 1 ? 'transparent' : C.gold, border: `1px solid ${idx === filtered.length - 1 ? C.border : C.gold}`, borderRadius: '5px', color: idx === filtered.length - 1 ? C.muted : '#0F1628', cursor: idx === filtered.length - 1 ? 'default' : 'pointer', fontSize: '0.875rem', fontWeight: 700 }}>
      ...
      {expanded && (
        <div onClick={() => setExpanded(null)}
          ...
          <div onClick={(e) => e.stopPropagation()}
            style={{ background: C.deep, border: `1px solid ${C.border}`, borderRadius: '10px', padding: '2rem', maxWidth: '600px', width: '100%', maxHeight: '80vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem' }}>
              <span style={{ padding: '3px 10px', background: 'rgba(194,163,104,0.12)', border: `1px solid ${C.border}`, borderRadius: '999px', color: C.gold, fontSize: '0.7rem' }}>{expanded.category}</span>
            ...
            <h2 style={{ fontFamily: 'var(--font-playfair)', color: C.text, fontSize: '1.4rem', fontWeight: 700, marginBottom: '1rem' }}>{expanded.title}</h2>
