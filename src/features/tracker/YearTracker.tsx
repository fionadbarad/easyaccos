...
interface Expense extends DatedAmount {
  id: string
}
...
export default function YearTracker() {
  ...
  const { items: transactions, loading: txLoading } = useUserData<Transaction>(
    'user_transactions',
    'ea_transactions',
    [],
  )
  const { items: expenses, loading: expLoading } = useUserData<Expense>(
    'user_expenses',
    'ea_expenses',
    [],
  )
    ...
    >
      ...
      >
        ...
        <button
          ...
          onMouseEnter={(e) => {
            ;(e.currentTarget as HTMLElement).style.borderColor = 'rgba(244,245,248,0.3)'
          ...
          >
            ...
            >
              <div>
                ...
                <Select
                  value={empType}
                  onChange={(v) => setEmpType(v as EmploymentType)}
                  options={[
              ...
              <div>
                ...
                <input
                  ...
                  style={inputStyle}
                  onFocus={(e) =>
                    ((e.target as HTMLInputElement).style.borderColor = 'rgba(244,245,248,0.3)')
                  }
      ...
      <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        <div>
          ...
          <div
            style={{ height: '4px', background: C.gray, borderRadius: '2px', overflow: 'hidden' }}
          >
            <motion.div
              initial={{ width: 0 }}
              ...
              style={{
                height: '100%',
                background: yearProgress > 0.75 ? C.amber : C.white,
                borderRadius: '2px',
              }}
            />
          </div>
              ...
              {[
                {
                  ...
                  sub: 'from logged transactions',
                },
                {
                  label: 'Expenses this year',
                  value: fmt(yearExpenses),
                  sub: 'from logged expenses',
                },
                {
                  label: 'Projected annual',
                  value: fmt(projectedIncome),
                  sub: `based on ${monthsElapsed.toFixed(1)}mo data`,
                },
                {
                  label: 'Projected net profit',
                  value: fmt(
                    Math.max(0, projectedIncome - projectedExpenses - monthlyPensionNum * 12),
                  ),
                  sub: 'after expenses & pension',
                },
              ].map(({ label, value, sub }) => (
                ...
                {projectedTax ? (
                  <>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                      ...
                      {projectedTax.niClass4 > 0 && (
                        <Row label="NI Class 4" value={fmt(projectedTax.niClass4)} />
                      )}
                      {projectedTax.niClass1 > 0 && (
                        <Row label="NI Class 1" value={fmt(projectedTax.niClass1)} />
                      )}
                      {projectedTax.studentLoanRepayment > 0 && (
                        <Row label="Student Loan" value={fmt(projectedTax.studentLoanRepayment)} />
                      )}
                      <div
                        style={{
                          display: 'flex',
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    value={potSaved}
                    onChange={(e) => setPotSaved(e.target.value)}
                    style={inputStyle}
                    onFocus={(e) =>
                      ((e.target as HTMLInputElement).style.borderColor = 'rgba(244,245,248,0.3)')
                    }
                  ...
                  <div>
                    <div style={{ marginBottom: '10px' }}>
                      ...
                      >
                        ...
                        >
                          {Math.min(
                            100,
                            Math.round((savedSoFar / Math.max(projectedBill, 1)) * 100),
                          )}
                          %
                        </span>
                      </div>
                      ...
                      >
                        <motion.div
                          animate={{
                            width: `${Math.min(100, (savedSoFar / Math.max(projectedBill, 1)) * 100)}%`,
                          }}
                          ...
                          style={{
                            ...
                            background: isOnTrack
                              ? C.green
                              : savedSoFar / Math.max(projectedBill, 1) > 0.7
                                ? C.amber
                                : C.red,
                            borderRadius: '3px',
                          }}
                    ...
                    >
                      {isOnTrack ? (
                        <CheckCircle
                          size={14}
                          style={{ color: C.green, flexShrink: 0, marginTop: '1px' }}
                        />
                      ) : gap > 0 ? (
                        <TrendingDown
                          size={14}
                          style={{ color: C.red, flexShrink: 0, marginTop: '1px' }}
                        />
                      ) : (
                        <Minus
                          size={14}
                          style={{ color: C.muted, flexShrink: 0, marginTop: '1px' }}
                        />
                      )}
                      <div>
                        {isOnTrack ? (
                            ...
                            <div style={{ color: C.muted, fontSize: T.caption, lineHeight: 1.5 }}>
                              {monthsRemaining > 0 ? (
                                <>
                                  ...
                                  for the next {Math.round(monthsRemaining)} months to cover your
                                  bill.
                                </>
                              ) : (
