...
export default function TaxPotCalculator() {
  ...
  const monthlyNI = result
    ? Math.round((result.niClass1 + result.niClass4 + result.niClass2) / 12)
    : 0
            ...
            {!hasResult ? (
              ...
              >
                ...
                >
                  <Row
                    label="Income Tax"
                    value={monthlyIncomeTax > 0 ? `${fmt(monthlyIncomeTax)}/mo` : '—'}
                  />
                  {result && result.niClass4 > 0 && (
                    <Row
                      label="NI Class 4 (SE)"
                      value={`${fmt(Math.round(result.niClass4 / 12))}/mo`}
                    />
                  )}
                  {result && result.niClass1 > 0 && (
                    <Row
                      label="NI Class 1 (PAYE)"
                      value={`${fmt(Math.round(result.niClass1 / 12))}/mo`}
                    />
                  )}
                  {result && result.niClass2 > 0 && (
                    <Row label="NI Class 2" value={`${fmt(Math.round(result.niClass2 / 12))}/mo`} />
                  )}
                  {result && result.studentLoanRepayment > 0 && (
                    <Row label="Student Loan" value={`${fmt(monthlySL)}/mo`} />
                  )}
                  <div
                    style={{
                      display: 'flex',
