...
function StatCard({ label, value, color = 'var(--sa-white)', sub }: { label: string; value: string; color?: string; sub?: string }) {
  return (
    <div className="bg-[var(--sa-surface)] border border-[var(--sa-border)] rounded-md px-[1.1rem] py-[1rem]">
      <div className="text-[rgba(244,245,248,0.18)] text-[0.58rem] uppercase tracking-[0.09em] font-semibold font-mono mb-1.5">{label}</div>
      <div className="text-[1.35rem] font-semibold tracking-[-0.03em] tabular-nums leading-[1]" style={{ color }}>{value}</div>
      {sub && <div className="text-[rgba(244,245,248,0.18)] text-[0.65rem] mt-1">{sub}</div>}
    </div>
  )
}
    ...
    <div className="p-[clamp(1.5rem,4vw,2.5rem)] max-w-[960px]">
      <div className="flex items-start justify-between flex-wrap gap-4 mb-7">
        <div>
          ...
          <h1 className="text-[var(--sa-white)] text-[clamp(1.4rem,3vw,1.9rem)] font-semibold tracking-[-0.03em] m-0 mb-1">
            Invoices
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowForm(o => !o)}
            className="flex items-center gap-1.5 bg-[var(--sa-white)] text-[var(--sa-black)] border-none rounded px-[16px] py-[8px] text-[0.8rem] font-semibold cursor-pointer tracking-[-0.01em]">
      ...
      <AnimatePresence>
        {stats.overdueCount > 0 && (
          <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="mb-5 px-[14px] py-[10px] bg-[rgba(248,113,113,0.06)] border border-[rgba(248,113,113,0.2)] rounded flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-2">
      ...
      <div className="flex gap-[0.35rem] mb-[0.85rem] flex-wrap">
        {FILTERS.map(({ key, label }) => (
          <button key={key} onClick={() => setFilter(key)}
            className={`px-[12px] py-[5px] rounded-[3px] border text-xs cursor-pointer transition-all duration-[100ms] ${filter === key ? 'border-[rgba(244,245,248,0.2)] bg-[var(--sa-border)] text-[var(--sa-white)] font-medium' : 'border-[var(--sa-border)] bg-transparent text-[rgba(244,245,248,0.42)] font-normal'}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div className="bg-[var(--sa-surface)] border border-[var(--sa-border)] rounded-md p-[3rem] text-center text-[rgba(244,245,248,0.42)] text-[0.84rem]">Loading…</div>
      ) : displayed.length === 0 ? (
        <div className="bg-[var(--sa-surface)] border border-[var(--sa-border)] rounded-md p-[3.5rem] text-center">
          <div className="text-[rgba(244,245,248,0.42)] text-[0.84rem] mb-1.5">{invoices.length === 0 ? 'No invoices yet.' : 'No invoices match this filter.'}</div>
