...
function StatCard({ label, value, sub, trend }: { label: string; value: string; sub?: string; trend?: 'up' | 'down' }) {
  return (
    <div className="bg-[var(--sa-surface)] border border-[var(--sa-border)] rounded-[6px] p-[1.1rem_1.25rem]">
...
export default function PnLPage() {
  ...
  return (
    <div className="p-[clamp(1.5rem,4vw,2.5rem)] max-w-[960px]">
      ...
      <div className="flex items-start justify-between flex-wrap gap-[1rem] mb-[1.5rem]">
        ...
        <div className="flex gap-[0.4rem] flex-wrap">
          ...
          <button onClick={exportJSON}
            ...
            className={`${btnBaseClass} flex items-center gap-[6px] border ${copied ? 'bg-[rgba(74,222,128,0.08)] border-[rgba(74,222,128,0.25)] text-[#4ADE80]' : copyError ? 'bg-[rgba(248,113,113,0.08)] border-[rgba(248,113,113,0.25)] text-[#F87171]' : 'bg-[var(--sa-surface)] border-[var(--sa-border)] text-[rgba(244,245,248,0.42)]'}`}>
            {copied ? <CheckCheck size={13} /> : <Copy size={13} />}
            {copied ? 'Copied' : copyError ? 'Clipboard blocked' : 'Export JSON'}
          </button>
          <button onClick={exportSA103CSV} title="SA103 self-assessment pre-fill CSV" className={`${btnBaseClass} flex items-center gap-[6px] bg-[var(--sa-surface)] border border-[var(--sa-border)] text-[rgba(244,245,248,0.42)]`}>
      ...
      {view === 'overview' && (
        <>
          <div className="grid gap-[1px] border border-[var(--sa-border)] rounded-[6px] overflow-hidden bg-[var(--sa-border)] mb-[1.5rem]"
          ...
          <div className="bg-[var(--sa-surface)] border border-[var(--sa-border)] rounded-[6px] p-[1.25rem] mb-[1rem]">
          ...
          <div className="bg-[var(--sa-surface)] border border-[var(--sa-border)] rounded-[6px] p-[1.25rem]">
      ...
      {view === 'income-statement' && (
        <div className="bg-[var(--sa-surface)] border border-[var(--sa-border)] rounded-[6px] p-[1.75rem] max-w-[580px]">
          ...
          <form onSubmit={addCogsEntry} className="grid gap-[6px] items-center m-[10px_0_4px] pl-[16px]"
            ...
            <input type="text" value={cogsForm.description} placeholder="e.g. Raw materials — steel"
              ...
              className="bg-[var(--sa-gray)] border border-[var(--sa-border)] rounded-[3px] p-[6px_8px] text-[var(--sa-white)] text-[0.75rem] outline-none" />
            <input type="number" min={0} step={0.01} value={cogsForm.amount} placeholder="0.00"
              onChange={(e) => setCogsForm((f) => ({ ...f, amount: e.target.value }))}
              className="bg-[var(--sa-gray)] border border-[var(--sa-border)] rounded-[3px] p-[6px_8px] text-[var(--sa-white)] text-[0.75rem] text-right font-mono outline-none" />
          ...
          <div className="bg-[rgba(244,245,248,0.04)] border border-[var(--sa-border)] rounded-[4px] p-[0.9rem_1rem] mt-[0.25rem]">
