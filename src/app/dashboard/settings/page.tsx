...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '8px', padding: '1.75rem', marginBottom: '1.25rem' }}>
      <h2 style={{ fontFamily: 'var(--font-playfair)', color: C.gold, fontSize: '1.05rem', fontWeight: 700, marginBottom: '1.4rem' }}>{title}</h2>
      {children}
    </div>
  )
}
  ...
  return (
    <div style={{ padding: 'clamp(1.5rem,4vw,2.5rem)', maxWidth: '680px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'rgba(194,163,104,0.18)', border: `2px solid ${C.gold}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.gold, fontSize: '1.3rem', fontWeight: 700 }}>
          {initial}
        </div>
        <div>
        ...
        <form onSubmit={saveProfile} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <button type="submit" disabled={saving}
              style={{ background: C.gold, color: C.deep, border: 'none', borderRadius: '4px', padding: '9px 22px', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', fontSize: '0.875rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
              {saving ? <Loader2 size={15} className="animate-spin" /> : null}
              {saving ? 'Saving…' : 'Save Profile'}
            </button>
            {saved && <span style={{ display: 'flex', alignItems: 'center', gap: '5px', color: C.gold, fontSize: '0.85rem' }}><CheckCircle size={15} /> Saved</span>}
          </div>
        </form>
      </Section>
      ...
      <Section title="Change Password">
        <form onSubmit={changePassword} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
          ...
          {pwMsg && (
            <div style={{ padding: '9px 13px', borderRadius: '4px', background: pwMsg.includes('success') ? 'rgba(100,200,100,0.1)' : 'rgba(220,53,69,0.1)', border: `1px solid ${pwMsg.includes('success') ? 'rgba(100,200,100,0.3)' : 'rgba(220,53,69,0.3)'}`, color: pwMsg.includes('success') ? '#80d680' : '#ff8a9a', fontSize: '0.85rem' }}>
              {pwMsg}
            </div>
          )}
          <button type="submit" disabled={pwSaving}
            style={{ background: C.gold, color: C.deep, border: 'none', borderRadius: '4px', padding: '9px 22px', fontWeight: 700, cursor: pwSaving ? 'not-allowed' : 'pointer', fontSize: '0.875rem', display: 'flex', alignItems: 'center', gap: '6px', width: 'fit-content' }}>
      ...
      <Section title="Account Info">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {[
            ...
            <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 0', borderBottom: `1px solid rgba(194,163,104,0.08)` }}>
              <span style={{ color: C.muted, fontSize: '0.875rem' }}>{label}</span>
              <span style={{ color: label === 'Plan' ? C.gold : C.text, fontSize: '0.875rem', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{value}</span>
            </div>
          ))}
        </div>
      </Section>
