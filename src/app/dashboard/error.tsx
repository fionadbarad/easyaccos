...
export default function DashboardError({
  ...
  return (
    <div className="p-[clamp(1.5rem,4vw,2.5rem)] max-w-[560px]">
      <div className="text-[#FBBF24] text-[0.62rem] uppercase tracking-[0.12em] mb-1.5 font-mono">
      ...
      {error.digest && (
        <pre className="text-[rgba(244,245,248,0.3)] text-[0.68rem] font-mono bg-[var(--sa-surface)] border border-[var(--sa-border)] rounded p-[0.6rem_0.8rem] mb-5">
          digest: {error.digest}
        </pre>
      )}
      <div className="flex gap-2 flex-wrap">
        <button onClick={reset}
          className="bg-[var(--sa-white)] text-[var(--sa-black)] border-none rounded p-[9px_16px] text-[0.82rem] font-semibold cursor-pointer tracking-[-0.01em]">
          Try again
        </button>
        <a href="mailto:support@easyacco.com?subject=Dashboard%20error"
          className="bg-transparent text-[rgba(244,245,248,0.7)] border border-[rgba(244,245,248,0.15)] rounded p-[9px_16px] text-[0.82rem] font-medium no-underline tracking-[-0.01em]">
          Contact support
        </a>
      </div>
    </div>
