...
|----|-----|----------|---------|---------------|
| SEC-1 | 🔴 | `src/lib/auth-shared.ts:13` | `getCachedUser` keys `unstable_cache` on `cookieStore.get('sb-access-token')` — a cookie name **@supabase/ssr never sets** (it uses `sb-<ref>-auth-token`). Key is always `'anonymous'`, so the first user's `User` object is cached and served to **every** visitor for 60s. Cross-user identity leak. | Never cache an authed user across requests. Drop `unstable_cache`; rely on the per-request React `cache()` only, or key on the real auth cookie. |
| SEC-2 | 🔴 | `middleware.ts:28`, `src/app/dashboard/layout.tsx:4` | Middleware only refreshes the session; layout fetches the user but **never redirects** on null. All `/dashboard/*` renders in guest mode. Data confidentiality rests **entirely** on Supabase RLS. | Verify RLS on every `user_*` + `audit_logs` table. Add a server redirect for routes that must be authed. |
| SEC-3 | 🟠 | `src/lib/use-user-data.ts:283, 331` | `.delete().in('id', …)` and `.upsert()` are not constrained by `.eq('user_id', userId)` — isolation is RLS-only. | Add explicit `user_id` filters as defence-in-depth. |
...
| ID | Sev | Location | Problem | Fix direction |
|----|-----|----------|---------|---------------|
| DAT-1 | 🔴 | `src/lib/use-user-data.ts:281` | Sync is last-write-wins full-table replace. `persist` closes over `items`; stale local state (2nd tab, failed load, auth race) makes `toDelete` include real rows → **server records deleted**. | Move to per-row upsert without a global delete, or a tombstone/soft-delete model. |
| AUD-1 | 🔴 | `src/lib/feature-flags.ts:15`, `src/lib/audit.ts:49` | Audit log is gated on `FLAG_AUDIT`, which **defaults OFF** (needs `NEXT_PUBLIC_EA_AUDIT` truthy). No env var → **no audit trail at all**. | Make audit mandatory for authed users; remove the off-by-default flag. |
| AUD-2 | 🟠 | `src/lib/audit.ts:57-79` | "Authoritative" store is client IndexedDB (user can edit/delete). Supabase mirror is fire-and-forget (`void`, no await/retry/error handling). **Not an immutable audit trail** — MTD/accounting needs server-authoritative, append-only. | Server writes the audit row (RPC/trigger) as source of truth; make it append-only (no update/delete policy). |
...
| TAX-5 | 🔴 | `src/lib/tax/bands-2026.ts` | Several constants are **unverified / likely invented** as HMRC 2026/27 fact: dividend `10.75% / 35.75%`, Class 2 `£3.65/wk`, SPT `£7,105`. Also embedded into the AI prompt and P&L. | **Owner to verify every figure against HMRC**, with source + effective date; label estimates as estimates. |
