...
function StatCard({ label, value, color = '#F4F5F8', sub }: { label: string; value: string; color?: string; sub?: string }) {
  return (
    <div className="bg-[#1C1D20] border border-[rgba(244,245,248,0.07)] rounded-[6px] px-[1.1rem] py-[1rem]">
    ...
    <div className="p-[clamp(1.5rem,4vw,2.5rem)] max-w-[960px]">
      <div className="flex items-start justify-between flex-wrap gap-[1rem] mb-[1.75rem]">
        <div>
          ...
          <h1 className="text-[#F4F5F8] text-[clamp(1.4rem,3vw,1.9rem)] font-semibold tracking-[-0.03em] m-0 mb-[4px]">
            Invoices
          </h1>
        ...
        <div className="flex items-center gap-[0.5rem]">
          ...
          <button onClick={() => setShowForm(o => !o)}
            className="flex items-center gap-[6px] bg-[#F4F5F8] text-[#181818] border-none rounded-[4px] px-[16px] py-[8px] text-[0.8rem] font-semibold cursor-pointer tracking-[-0.01em]">
      ...
      <div className="flex gap-[0.35rem] mb-[0.85rem] flex-wrap">
        {FILTERS.map(({ key, label }) => (
          <button key={key} onClick={() => setFilter(key)}
            className={`px-[12px] py-[5px] rounded-[3px] border text-[0.75rem] cursor-pointer transition-all duration-[100ms] ${filter === key ? 'border-[rgba(244,245,248,0.2)] bg-[rgba(244,245,248,0.07)] text-[#F4F5F8] font-medium' : 'border-[rgba(244,245,248,0.07)] bg-transparent text-[rgba(244,245,248,0.42)] font-normal'}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div className="bg-[#1C1D20] border border-[rgba(244,245,248,0.07)] rounded-[6px] p-[3rem] text-center text-[rgba(244,245,248,0.42)] text-[0.84rem]">Loading…</div>
      ) : displayed.length === 0 ? (
        <div className="bg-[#1C1D20] border border-[rgba(244,245,248,0.07)] rounded-[6px] p-[3.5rem] text-center">
